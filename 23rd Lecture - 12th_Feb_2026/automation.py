...
class PDFToMilvusAutomation:
    ...
    async def generate_responses(self, search_result):
        ...
        # ])
        responses = await asyncio.gather(*[
            get_qwen.qwen_response(prompt) for prompt in prompts.values()
        ])
        ...
        lit_review = await get_qwen.qwen_response(get_prompt.prompt_for_lit_review(response_data['reference']))
        ...
        if caption_prompt:
            ...
            caption = await get_qwen.qwen_response(get_prompt.prompt_for_caption(caption_prompt))
